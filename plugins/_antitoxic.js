const isToxic = /anj(k|g)|ajn?(g|k)|a?njin(g|k)|bajingan|b(a?n)?gsa?t|ko?nto?l|me?me?(k|q)|pe?pe?(k|q)|meki|titi(t|d)|pe?ler|tetek|toket|ngewe|go?blo?k|to?lo?l|idiot|(k|ng)e?nto?(t|d)|jembut|bego|dajj?al|janc(u|o)k|pantek|puki ?(mak)?|kimak|kampang|lonte|col(i|mek?)|pelacur|henceu?t|nigga|fuck|dick|bitch|tits|bastard|asshole|a(su|sw|syu)anjing|kontol|bangsat/i // tambahin sendiri
...
export async function before(m, { conn, args, usedPrefix, command, isAdmin, isBotAdmin }) {
    ...
    if (chat.antiToxic && isAntiToxic) {
        await conn.sendButton(m.chat, `*Jangan toxic adik adik 👿*${isBotAdmin ? '' : '\n\n_Bot bukan atmin_'}`, author, ['off antitoxic', '/disable antitoxic'], m)
    ...
    await conn.sendButton(m.chat, `*Anda Bocah PKI*
